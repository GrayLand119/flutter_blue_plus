...
class FlutterBluePlus {
  ...
  Stream<BluetoothState> get state async* {
    BluetoothState initialState =
        await _channel.invokeMethod('state').then((buffer) => protos.BluetoothState.fromBuffer(buffer)).then((s) => BluetoothState.values[s.state.value]);
  ...
  Stream<ScanResult> scan({
    ...
    if (_updateInterval != null) {
      _updateScanResultTimer = Timer.periodic(_updateInterval!, (t) {
        ...
        // For fixing situations that may cause an error:
        ...
        if (_expiredInterval != null) {
          ...
          _cachedDevices.removeWhere((key, value) {
            // print("cached device check: ${(nowSec - value.date.millisecondsSinceEpoch) > diffTime}");
            if ((nowSec - value.date.millisecondsSinceEpoch) > diffTime) {
              return true;
            }
            // print("Remove expired device :$key diffTime:${diffTime / 1000.0} diff:${(nowSec - value.date.millisecondsSinceEpoch) / 1000.0}");
            return false;
          });
        }
